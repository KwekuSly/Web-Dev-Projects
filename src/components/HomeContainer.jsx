    ...
    <section className="grid grid-cols-1 md:grid-cols-2 gap-2 w-full" id="home">
      <div className="py-2 flex-1 flex flex-col items-start justify-center gap-6">
        ...
        <p className="text-[2.5rem] lg:text-[3rem] font-bold tracking-wide text-headingColor">
          ...
          <span className="text-green-600 text-[3rem] lg:text-[4rem]">
            {" "}
            In Ghana
          </span>
        </p>
        <p className="text-base text-textColor text-center md:text-left md:w-[80%]">
          Your One-Stop Online Grocery Shop in Ghana: Fresh, Convenient,
          Delivered to Your Doorstep!
        </p>

        <button
          type="button"
          className="bg-gradient-to-br from-green-400 to-green-500 md:w-auto w-full px-4 py-2 rounded-lg hover:shadow-lg transition-all ease-in-out duration-100"
        >
          Order Now
        </button>
      </div>
          ...
          {heroData &&
            heroData.map((n) => (
              ...
              >
                ...
                <p className="text-[10px]  lg:text-sm text-lighttextGray font-semibold  my-1 lg:my-1">
                  {n.decp}
                </p>

                <p className="text-sm font-semibold text-headingColor">
                  <span className="text-xs text-green-500">Ghc</span>
                  {n.price}
                </p>
              </div>
            ))}
